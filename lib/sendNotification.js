...
/**
 * Send only a push notification to a user's device — does NOT save
 * anything to the Notification collection / in-app notifications list.
 *
 * Use this for things that already have their own dedicated UI and
 * unread-tracking (e.g. chat messages, which live in /chat and use
 * Message.seen, not the Notification model).
 *
 * @param {string} userId
  ...
  try {
    ...
    if (sub) {
      await webpush.sendNotification(
        sub.subscription,
        JSON.stringify({ title, body, url, unreadCount }),
        {
          urgency: 'high',
          TTL:     300,
        }
    ...
    console.error('sendPushOnly error:', error.message);
  }
}
 ...
 *
 * Use this for everything EXCEPT chat messages — job applications,
 * verification updates, admin announcements, account status changes, etc.
 *
